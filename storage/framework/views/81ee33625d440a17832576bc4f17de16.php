<?php $__env->startSection('page-title'); ?>
    <?php echo e(__('Create Employee')); ?>

<?php $__env->stopSection(); ?>

<?php $__env->startSection('breadcrumb'); ?>
    <li class="breadcrumb-item"><a href="<?php echo e(route('dashboard')); ?>"><?php echo e(__('Home')); ?></a></li>
    <li class="breadcrumb-item"><a href="<?php echo e(url('employee')); ?>"><?php echo e(__('Employee')); ?></a></li>
    <li class="breadcrumb-item"><?php echo e(__('Create Employee')); ?></li>
<?php $__env->stopSection(); ?>


<?php $__env->startSection('content'); ?>
    <div class="row">
        <div class="">
            <div class="">
                <div class="row">
                </div>
                <?php echo e(Form::open(['route' => ['employee.store'], 'method' => 'post', 'enctype' => 'multipart/form-data'])); ?>

                <div class="row mb-3">

                    <div class="col-md-12 mb-3">
                        <div class="card em-card">
                            <div class="card-header">
                                <h5><?php echo e(__('Employee Data')); ?></h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="form-group mb-2 ">
                                        <?php echo Form::label('employee_id', __('Employee ID'), ['class' => 'form-label']); ?>

                                        <?php echo Form::text('employee_id', $employeesId, ['class' => 'form-control', 'disabled' => 'disabled']); ?>

                                    </div>
                                    <div class="form-group col-md-6 mb-2">
                                        <?php echo Form::label('name', __('Name'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                        <?php echo Form::text('name', old('name'), [
                                            'class' => 'form-control',
                                            'required' => 'required',
                                            'placeholder' => 'Enter employee name',
                                        ]); ?>

                                    </div>
                                    <div class="form-group col-md-6 mb-2">
                                        <?php echo Form::label('phone', __('Phone'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                        <?php echo Form::text('phone', old('phone'), [
                                            'class' => 'form-control',
                                            'required' => 'required',
                                            'placeholder' => 'Enter employee phone',
                                        ]); ?>

                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <div class="form-group">
                                            <?php echo Form::label('dob', __('Date of Birth'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                            <?php echo e(Form::date('dob', null, ['class' => 'form-control ', 'required' => 'required', 'autocomplete' => 'off', 'placeholder' => 'Select Date of Birth'])); ?>

                                        </div>
                                    </div>
                                    <div class="col-md-6 mb-2">
                                        <div class="form-group">
                                            <?php echo Form::label('gender', __('Gender'), ['class' => 'form-label', 'required' => 'required']); ?><span class="text-danger pl-1">*</span>
                                            <div class="d-flex radio-check">
                                                <div class="custom-control custom-radio custom-control-inline">
                                                    <input type="radio" id="g_male" value="Male" name="gender"
                                                        class="form-check-input" checked>
                                                    <label class="form-check-label "
                                                        for="g_male"><?php echo e(__('Male')); ?></label>
                                                </div>
                                                <div class="custom-control custom-radio ms-1 custom-control-inline">
                                                    <input type="radio" id="g_female" value="Female" name="gender"
                                                        class="form-check-input">
                                                    <label class="form-check-label "
                                                        for="g_female"><?php echo e(__('Female')); ?></label>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="form-group col-md-12 mb-2">
                                        <?php echo Form::label('email', __('Email'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                        <?php echo Form::email('email', old('email'), [
                                            'class' => 'form-control',
                                            'required' => 'required',
                                            'placeholder' => 'Enter employee email',
                                        ]); ?>

                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-4">
                                        <div class="form-group mb-2">
                                            <?php echo Form::label('address', __('Address'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                            <?php echo Form::textarea('address', old('address'), [
                                                'class' => 'form-control',
                                                'rows' => 2,
                                                'placeholder' => 'Enter employee address',
                                                'required' => 'required',
                                            ]); ?>

                                        </div>
                                    </div>

                                    <div class="col-4">
                                        <div class="form-group mb-2">
                                            <?php echo Form::label('city', __('City'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                            <?php echo Form::text('city', old('city'), [
                                                'class' => 'form-control',
                                                'rows' => 2,
                                                'placeholder' => 'Enter employee city',
                                                'required' => 'required',
                                            ]); ?>

                                        </div>
                                    </div>
                                    <div class="col-4">
                                        <div class="form-group mb-2">
                                            <?php echo Form::label('country', __('Country'), ['class' => 'form-label']); ?><span class="text-danger pl-1">*</span>
                                            <?php echo Form::text('country', old('country'), [
                                                'class' => 'form-control',
                                                'rows' => 2,
                                                'placeholder' => 'Enter employee country',
                                                'required' => 'required',
                                            ]); ?>

                                        </div>
                                    </div>

                                </div>

                            </div>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="card em-card">
                            <div class="card-header">
                                <h5><?php echo e(__('Company Data')); ?></h5>
                            </div>
                            <div class="card-body employee-detail-create-body">
                                <div class="row">
                                    <?php echo csrf_field(); ?>
                                    <div class="row">
                                        <div class="form-group col-4 mb-2 ">
                                            <div class="d-flex justify-content-between">
                                                <?php echo e(Form::label('position_id', __('Select Position*'), ['class' => 'form-label'])); ?>

                                                <button type="button" id="addNewPosition" class="btn btn-primary btn-sm mb-3">
                                                    <?php echo e(__('Add New Position')); ?>

                                                </button>
                                            </div>

                                            <div class="form-icon-user">
                                                <?php echo e(Form::select('position_id', $positions, null, ['class' => 'form-control', 'id' => 'position_id', 'placeholder' => 'Select Position'])); ?>

                                            </div>


                                            <div id="myModal" class="modal fade" tabindex="-1">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title"><?php echo e(__('Add New Position')); ?></h5>
                                                            <button type="button" class="close btn btn-primary btn-sm "
                                                                data-dismiss="modal">&times;</button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <form method="POST" id="addNewPositionForm">
                                                                <?php echo csrf_field(); ?>
                                                                <div class="row">

                                                                    <div class="col-12">
                                                                        <div class="form-group mb-2">
                                                                            <?php echo e(Form::label('name', __('Name'), ['class' => 'form-label'])); ?>

                                                                            <?php echo e(Form::text('name', null, ['class' => 'form-control', 'placeholder' => __('Enter Position Name'), 'id' => 'position_name'])); ?>

                                                                            <?php $__errorArgs = ['name'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                                                                                <span class="invalid-name" role="alert">
                                                                                    <strong
                                                                                        class="text-danger"><?php echo e($message); ?></strong>
                                                                                </span>
                                                                            <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
                                                                        </div>
                                                                        <div class="form-group mb-2">
                                                                            <?php echo e(Form::label('descriptions', __('Descriptions'), ['class' => 'form-label'])); ?>

                                                                            <?php echo e(Form::textarea('descriptions', null, ['class' => 'form-control', 'placeholder' => __('Enter Position Descriptions'), 'id' => 'position_descriptions'])); ?>

                                                                            <?php $__errorArgs = ['descriptions'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                                                                                <span class="invaliD-name" role="alert">
                                                                                    <strong
                                                                                        class="text-danger"><?php echo e($message); ?></strong>
                                                                                </span>
                                                                            <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
                                                                        </div>
                                                                        <div class="form-group">
                                                                            <?php echo e(Form::label('slaray_range', __('Salary Range'), ['class' => 'form-label'])); ?>

                                                                            <?php echo e(Form::text('slaray_range', null, ['class' => 'form-control', 'placeholder' => __('Enter Salary Range'), 'id' => 'position_slaray_range'])); ?>

                                                                            <?php $__errorArgs = ['slaray_range'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                                                                                <span class="invalid-name" role="alert">
                                                                                    <strong
                                                                                        class="text-danger"><?php echo e($message); ?></strong>
                                                                                </span>
                                                                            <?php unset($message);
if (isset($__messageOriginal)) { $message = $__messageOriginal; }
endif;
unset($__errorArgs, $__bag); ?>
                                                                        </div>
                                                                    </div>

                                                                </div>
                                                                <button type="button" onclick="submitPositionForm()"
                                                                    class="btn btn-primary mt-3"><?php echo e(__('Create')); ?></button>
                                                            </form>
                                                        </div>

                                                    </div>
                                                </div>
                                            </div>

                                        </div>

                                        <div class="form-group col-4 mb-2">
                                            <?php echo e(Form::label('department_id', __('Select Department*'), ['class' => 'form-label'])); ?>

                                            <div class="form-icon-user">
                                                <?php echo e(Form::select('department_id', $departments, null, ['class' => 'form-control', 'id' => 'department_id', 'placeholder' => 'Select Department'])); ?>

                                            </div>
                                        </div>

                                        <div class="form-group col-4 mb-2 ">
                                            <?php echo e(Form::label('designation_id', __('Select Section*'), ['class' => 'form-label'])); ?>


                                            <div class="form-icon-user">
                                                <?php echo e(Form::select('designation_id', $designations, null, ['class' => 'form-control', 'id' => 'designation_id', 'placeholder' => 'Select Section'])); ?>

                                            </div>
                                        </div>


                                    </div>

                                    <div class="row">
                                        <div class="form-group mb-2 col-6">
                                            <?php echo Form::label('company_doj', __('Hire Date'), ['class' => '  form-label']); ?>

                                            <?php echo e(Form::date('company_doj', null, ['class' => 'form-control ', 'required' => 'required', 'autocomplete' => 'off', 'placeholder' => 'Select Hire Date'])); ?>

                                        </div>

                                        <div class="form-group mb-2 col-6">
                                            <?php echo Form::label('contract_end_date', __('Contract End Date'), ['class' => '  form-label']); ?>

                                            <?php echo e(Form::date('contract_end_date', null, ['class' => 'form-control ', 'required' => 'required', 'autocomplete' => 'off', 'placeholder' => 'Select Contract End Date'])); ?>

                                        </div>
                                    </div>

                                    <div class="form-group mb-2  ">
                                        <?php echo Form::label('salary', __('Salary'), ['class' => '  form-label']); ?>

                                        <?php echo e(Form::number('salary', null, ['class' => 'form-control ', 'required' => 'required', 'autocomplete' => 'off', 'placeholder' => 'Employee Salary'])); ?>

                                    </div>

                                    <div class="row">
                                        <div class="form-group col-md-6 mb-2">
                                            <?php echo e(Form::label('employee_status', __('Select Status*'), ['class' => 'form-label'])); ?>

                                            <div class="form-icon-user">
                                                <?php echo e(Form::select(
                                                    'employee_status',
                                                    [
                                                        '1' => __('Active'),
                                                        '2' => __('Terminated'),
                                                    ],
                                                    null,
                                                    ['class' => 'form-control', 'id' => 'employee_status', 'placeholder' => 'Select Status'],
                                                )); ?>

                                            </div>
                                        </div>

                                        <div class="form-group col-md-6 mb-2">
                                            <?php echo e(Form::label('contract_type', __('Select Contract Type*'), ['class' => 'form-label'])); ?>

                                            <div class="form-icon-user">
                                                <?php echo e(Form::select(
                                                    'contract_type',
                                                    [
                                                        '1' => __('Full Time'),
                                                        '2' => __('Part Time'),
                                                    ],
                                                    null,
                                                    ['class' => 'form-control', 'id' => 'contract_type', 'placeholder' => 'Select Contract Type'],
                                                )); ?>

                                            </div>
                                        </div>
                                    </div>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>

            <div class="float-end">
                <button type="submit" class="btn  btn-primary"><?php echo e('Create'); ?></button>
            </div>
            </form>
        </div>
    </div>
<?php $__env->stopSection(); ?>


<?php $__env->startPush('script-page'); ?>
    <script>
        $(document).ready(function(){
            $('#addNewPosition').on('click',function(){
                $("#myModal").modal('show');
            })
    
            // Close modal on button click
            
        });
        
        function submitPositionForm() {
            $.ajax({
                url: '<?php echo e(route('new.positions.ajax')); ?>',
                type: 'POST',
                data: {
                    position_name: $('#position_name').val(),
                    position_descriptions: $('#position_descriptions').val(),
                    position_slaray_range: $('#position_slaray_range').val(),
                    "_token": "<?php echo e(csrf_token()); ?>",
                },
                success: function(response) {
                    var select = document.getElementById("position_id");
                    var option = document.createElement("option");
                    option.value = response.data.id;
                    option.text = response.data.name;
                    select.appendChild(option);
                    $("#myModal").modal('hide');
                },
                error: function(xhr, status, error) {
                    // Handle error
                    console.error(error);
                }
            });
        }
    </script>

    <script>
        $('input[type="file"]').change(function(e) {
            var file = e.target.files[0].name;
            var file_name = $(this).attr('data-filename');
            $('.' + file_name).append(file);
        });
    </script>
    <script>
        $(document).ready(function() {
            var d_id = $('.department_id').val();
            getDesignation(d_id);
        });

        $(document).on('change', 'select[name=department_id]', function() {
            var department_id = $(this).val();
            getDesignation(department_id);
        });

        function getDesignation(did) {

            $.ajax({
                url: '<?php echo e(route('employee.json')); ?>',
                type: 'POST',
                data: {
                    "department_id": did,
                    "_token": "<?php echo e(csrf_token()); ?>",
                },
                success: function(data) {

                    $('.designation_id').empty();
                    var emp_selct = ` <select class="form-control  designation_id" name="designation_id" id="choices-multiple"
                                            placeholder="Select Designation" >
                                            </select>`;
                    $('.designation_div').html(emp_selct);

                    $('.designation_id').append('<option value="0"> <?php echo e(__('All')); ?> </option>');
                    $.each(data, function(key, value) {
                        $('.designation_id').append('<option value="' + key + '">' + value +
                            '</option>');
                    });
                    new Choices('#choices-multiple', {
                        removeItemButton: true,
                    });


                }
            });
        }
    </script>
<?php $__env->stopPush(); ?>

<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\rezo2\resources\views/employee/create.blade.php ENDPATH**/ ?>