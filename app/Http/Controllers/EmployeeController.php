...
class EmployeeController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create employee')) {
            ...
            $positions     = Position::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            ...
            return view('employee.create', compact('employees', 'employeesId', 'departments', 'designations', 'documents', 'branches', 'company_settings', 'positions', 'nextId'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit employee')) {
            ...
            return view('employee.edit', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents', 'departmentData', 'positions'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    public function show($id)
    {
        ...
        if (\Auth::user()->can('view employee')) {
            ...
            return view('employee.show', compact('employee', 'employeesId', 'branches', 'departments', 'designations', 'documents', 'positions'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    {
        $latest = Employee::where('created_by', '=', \Auth::user()->creatorId())->latest()->value('id');
        if (!$latest) {
            return 1;
        }
    ...
    public function import(Request $request)
    {
        ...
        for ($i = 1; $i <= count($employees) - 1; $i++) {
            ...
            // dd(count($employee));
            if (count($employee) >= 15) {
                if ($employee[4] == null) {
                    return redirect()->back()->with('error', __('Email Filed is Required'));
                }
                $employeeByEmail = Employee::where('email', $employee[4])->first();
                // dd($employeeByEmail);
                $userByEmail = User::where('email', $employee[4])->first();
                // dd($userByEmail);
                ...
                $employeeData->phone = $employee[1];
                $employeeData->dob = $employee[2];
                $employeeData->gender = $employee[3];
                $employeeData->email = $employee[4];
                $employeeData->address = $employee[5];
                $employeeData->city = $employee[6];
                $employeeData->country = $employee[7];
                $employeeData->position_id = $employee[8];
                $employeeData->department_id = $employee[9];
                $employeeData->designation_id = $employee[10];
                $employeeData->company_doj = $employee[11];
                $employeeData->contract_end_date = $employee[12];
                $employeeData->employee_status = $employee[13];
                $employeeData->contract_type = $employee[14];
