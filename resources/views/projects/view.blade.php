...
@section('content')
    @php
        $project_position_count = \App\Models\ProjectPosition::where('project_id', $project->id)->sum(
            'position_employees_number',
        );
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row align-items-center justify-content-between">
                            <div class="col-auto mb-3 mb-sm-0">
                                <div >
                                    <span><strong>{{ __('Total Positions That Need').' : '.$project->positions }}</strong></span><br>
                                    <span><strong>{{ __('Assigned Positions').' : '.$project_position_count }}</strong></span><br>
                                    <span><strong>{{ __('Remain Positions').' : '.$project->positions - $project_position_count }}</strong></span><br>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div >
                                    @if (count($project->project_employees) > 0)
                                        @foreach ($project->project_employees as $emp)
                                            <span><strong>{{ __('Position').' : '.$emp->position->name }} | {{ __('Employee').' : '.$emp->employee->name.' | '.$emp->count }}</strong></span><br>
                                        @endforeach
                                    @else
        ...
        <div class="col-lg-4 col-md-4">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <div class="d-flex align-items-center">
                            <span class="text-muted">{{ __('Employee Assigned') }}</span>
                        </div>
                        <span>{{ $project_position_employee_count }}</span>
                    </div>
                    <div class="progress mb-3">
                        <div class="progress-bar bg-primary"
                            style="width: {{ ($project_position_count / $project->positions) * 100 }}%"></div>
                    </div>
                </div>
            </div>
                        ...
                        <li class="list-group-item px-0">
                            <div class="row align-items-center justify-content-between">
                                <div class="col-sm-auto mb-3 mb-sm-0">
                                    <div class="d-flex align-items-center">
                                        <div class="div">
                                            <h5 class="m-0">{{ $data->employee->name ?? '' }}</h5>
                                            <small class="text-muted">{{ $data->position->name ?? '' }}</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-auto text-sm-end d-flex align-items-center">
                                    <a href="{{ route('projects.deleteEmployees',['id'=>$data->id]) }}" onclick="return confirm('Are You Sure?')" class="mx-3 btn btn-sm btn-danger align-items-center"><i class="fa fa-trash text-white"></i></a>
                                </div>
                            </div>
                        </li>
                    @endforeach
